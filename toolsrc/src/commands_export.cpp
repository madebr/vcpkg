...
{
    using Install::InstallationDirs;
    ...
    void perform_and_exit(const VcpkgCmdArguments& args, const VcpkgPaths& paths, const Triplet& default_triplet)
    {
        ...
        for (const ExportPlanAction& action : export_plan)
        {
            ...
            const InstallationDirs dirs = InstallationDirs::from_destination_root(
                paths.package_dir(action.spec),
                output,
                action.spec.triplet().to_string(),
                output / "vcpkg" / "info" / (binary_paragraph.fullstem() + ".list"));

            Install::install_files_and_write_listfile(paths.get_filesystem(), dirs);
